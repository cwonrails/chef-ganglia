...
udp_send_channel {
  ...
  ttl = 1
    ...
    it 'gmond.conf does not spoof hostname' do
      expect(chef_run).to_not render_file('/etc/ganglia/gmond.conf').with_content(%(  override_hostname = ))
    end
  ...
  context 'unicast mode with hostname spoofing' do
    ...
    it 'gmond.conf does spoof hostname' do
      expect(chef_run).to render_file('/etc/ganglia/gmond.conf').with_content(%(  override_hostname = ))
    end
  end
  ...
  ttl = 1
    ...
    it 'writes the gmond.conf' do
      expect(chef_run).to create_template('/etc/ganglia/gmond.conf').with(
        variables: {
          cluster_name: 'default',
          gmond_collectors: %w(host1 host2),
          ports: [18_649],
          spoof_hostname: false,
          hostname: 'Fauxhai'
        }
